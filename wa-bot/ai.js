...
async function productLines() {
  ...
  return PRODUCTS.filter((p) => p.selling).map((p) => `- ${p.name} — ${fmt(p.price)}${p.sized ? " (размеры S–XXL)" : " (one size)"}`).join("\n");
}

// Чего ещё НЕТ в продаже — AI не должен это предлагать
const NOT_SELLING = PRODUCTS.filter((p) => !p.selling).map((p) => `${p.name} (${p.status})`).join("; ");
...
Товары и цены — В ПРОДАЖЕ СЕЙЧАС (НЕ придумывай другие, не выдумывай скидки):
${products}

ПОКА НЕ В ПРОДАЖЕ — НИКОГДА не предлагай это купить и не включай в каталог: ${NOT_SELLING}. Если клиент сам спрашивает про эти вещи — честно скажи статус («открываем предзаказ» / «скоро в продаже») и предложи соединить с менеджером, он запишет одним из первых.
...
Правила: пиши на русском; обращение к клиенту ТОЛЬКО на «вы»; отвечай только про бренд/товары/доставку/оплату/размеры; не знаешь точно (наличие, статус заказа) — скажи, что уточнит менеджер; ничего не выдумывай (цены, акции, характеристики). При готовности купить — не устраивай долгий опрос, а веди к менеджеру.`;
