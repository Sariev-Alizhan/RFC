...
export const T = {
  welcome:
    `Йо! 🚩 Ты в *${SHOP.brand}* — локальный бренд из Казахстана, тот самый red flag, который не хочется игнорить 😏\n\n` +
    `Я твой личный продажник. Помогу выбрать и оформить за пару сообщений. Что глянем?\n\n` +
    `• *Каталог* — что есть и почём\n` +
    `• *Размеры* — подберу под тебя\n` +
    `• *Доставка* — сроки и самовывоз\n` +
    `• *Оплата* — Kaspi, всё просто\n` +
    `• *Заказать* — погнали оформлять 🔥\n\n` +
    `Сайт: ${SHOP.site}`,
